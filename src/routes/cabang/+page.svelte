  ...
  async function handleToggleStatus(cabang: any, newChecked: boolean) {
    ...
    try {
      ...
      await apiClient.patch(
        `${API_ENDPOINTS.CABANG.UPDATE_STATUS(cabang.id)}`,
        {
          status: newStatus,
        },
      );
...
<div class="max-w-full mx-auto">
  <Card>
    <CardHeader
      ...
      description="Lihat dan kelola seluruh cabang dan status operasionalnya."
      icon={Building2}
    ...
    <CardContent class="pb-3">
      ...
      >
        {#snippet extraActions()}
          <Button
            variant="primary"
            onclick={() => goto(ROUTES.CABANG.CREATE)}
              ...
              <TableRow>
                ...
                <TableCell>
                  <div class="flex items-center gap-2">
                    <ToggleSwitch
                      size="sm"
                      checked={cabang.status?.toLowerCase() === "aktif"}
                      isLoading={togglingIds.has(cabang.id)}
                      onchange={(newChecked) =>
                        handleToggleStatus(cabang, newChecked)}
                    />
                    <span
                      class={`text-xs font-semibold uppercase tracking-wide ${cabang.status?.toLowerCase() === "aktif" ? "text-emerald-600" : "text-gray-400"}`}
                    >
                ...
                <TableCell class="text-right">
                  <Dropdown width="w-36" align="right">
                    ...
                    <DropdownItem
                      icon={Pencil}
                      onclick={() => goto(ROUTES.CABANG.EDIT(cabang.id))}
                    ...
                    <DropdownItem
                      icon={Trash2}
                      variant="danger"
                      onclick={() => confirmDelete(cabang.id)}
                      >Hapus</DropdownItem
                    >
                  </Dropdown>
                </TableCell>
            ...
            {:else}
              <TableRow>
                <TableCell colspan={5} class="h-40 text-center text-gray-500">
                  ...
                  {:else}
                    <span class="text-sm"
                      >Tidak ada data cabang yang ditemukan.</span
                    >
                  {/if}
                </TableCell>
              </TableRow>
            ...
            {#snippet actions()}
              <Dropdown width="w-32" align="right">
                ...
                <DropdownItem
                  icon={Trash2}
                  variant="danger"
                  onclick={() => confirmDelete(cabang.id)}>Hapus</DropdownItem
                >
              </Dropdown>
            {/snippet}
          </EntityCard>
...
{#if showDeleteModal}
  <Modal title="Konfirmasi Hapus" open={showDeleteModal}>
    <p class="text-sm text-gray-600 mb-6">
      Apakah Anda yakin ingin menghapus data cabang ini? Tindakan ini tidak
      dapat dibatalkan.
    </p>
