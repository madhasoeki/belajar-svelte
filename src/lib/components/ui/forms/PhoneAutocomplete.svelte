<script lang="ts">
  ...
  interface AutocompleteProps {
    ...
    onSelectDonatur: (donatur: { id: string; nama: string; sapaan: string; phone: string }) => void;
  ...
  let options = $state<Array<{ id: string, nomor_hp_donatur: string, nama_donatur: string, sapaan: string }>>([]);
  ...
  $effect(() => {
    ...
    timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        const res = await apiClient.get(`${API_ENDPOINTS.DONATUR.SEARCH}?q=${value}`);
  ...
  function handleSelect(opt: { id: string, nomor_hp_donatur: string, nama_donatur: string, sapaan: string }) {
    ...
    onSelectDonatur({
      id: opt.id,
      nama: opt.nama_donatur,
      sapaan: opt.sapaan || "Kak",
      phone: opt.nomor_hp_donatur
    });
  }
...
<div class={`flex flex-col gap-1.5 w-full ${wrapperClass}`} bind:this={container}>
  ...
  <div class="relative">
    <Phone size={18} class="absolute left-3 top-1/2 -translate-y-1/2 text-(--color-text-muted)" />
    ...
    {#if isLoading}
      <Loader2 size={16} class="absolute right-3 top-1/2 -translate-y-1/2 text-(--color-primary) animate-spin" />
    {/if}
    ...
    {#if isOpen && !isLoading && options.length > 0}
      <div class="absolute z-50 w-full mt-1 bg-white border border-(--color-border) rounded-lg shadow-lg overflow-hidden flex flex-col">
        <ul class="max-h-60 overflow-y-auto p-1">
          <div class="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50/50">
          ...
          {#each options as opt (opt.id)}
            ...
            >
              <span class="font-bold text-gray-900 group-hover:text-(--color-primary)">
                {opt.sapaan} {opt.nama_donatur}
              </span>
